...
var App = function() {
    ...
    var uiInit = function() {
        ...
        var exampleTypeheadData = ["Courts", "Polytach Mara", "Taq Bin", "SunLIght", "Shell", "Puspakom", "The Store", "Hospital Pantai", "CMC Cheras", "PPR Laksamana"];
        ...
        $('.input-datepicker, .input-daterange, .input-datepicker .input-group-addon').datepicker({ weekStart: 1, autoclose: true, format: 'dd/mm/yyyy' });
        $('.input-datepicker-month').datepicker({ weekStart: 1, autoclose: true, format: 'mm', viewMode: "months", minViewMode: "months" });
        $('.input-datepicker-year').datepicker({ weekStart: 1, autoclose: true, format: 'yyyy', viewMode: "years", minViewMode: "years" });
        $('.input-datepicker-close').datepicker({ weekStart: 1 }).on('changeDate', function (e) { $(this).datepicker('hide'); });
        $('.input-datepicker-find-age').datepicker({ weekStart: 1, autoclose: true, format: 'dd/mm/yyyy' }).on('changeDate', function (e) 
        {
            var date = $('#DOB').val().split('/');
            var dob = new Date(date[2], date[1] - 1, date[0]);
            var today = new Date();
            var age = Math.floor((today - dob) / (365.25 * 24 * 60 * 60 * 1000));
            var ageExact = isNaN(age) ? 0 : age;
            $('#CandidateAge').html(ageExact + ' years');
        });


        // Initialize Timepicker
        $('.input-timepicker12').timepicker({minuteStep: 1});
        $('.input-timepicker').timepicker({minuteStep: 1,showSeconds: true,showMeridian: false,defaultTime:false});
